...
bool LevelInputManager::handleInput(Uint32 time, const SDL_Event& event) {
	...
	if (!player->screenButtons_.empty())
	{
		...
		{
			if ((event.type == SDL_CONTROLLERBUTTONDOWN || event.type == SDL_KEYDOWN) && keyup2 &&( SDL_GameControllerGetButton(controller, it->getKey()) || (ControllerMode == 2 && it->getKey() == arrows)))
			{
