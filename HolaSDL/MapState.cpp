...
void MapState::createMainButtons()	//creates all the buttons needed for the levels
{
	...
	buttons[2].first = EmptyObject(manager, Vector2D(manager->getDefaultWindowWidth() / 2 - 60 , manager->getDefaultWindowHeight() / 2 - 30), 64, 64, Resources::MetroOn);
	buttons[0].first = EmptyObject(manager, Vector2D(manager->getDefaultWindowWidth() / 2 - 260, manager->getDefaultWindowHeight() / 2 - 165), 64, 64, Resources::MetroOff);
	buttons[3].first = EmptyObject(manager, Vector2D(manager->getDefaultWindowWidth() / 2 + 25 , manager->getDefaultWindowHeight() / 2 + 100), 64, 64, Resources::MetroOff);
	...
	buttons[1].first = EmptyObject(manager, Vector2D(manager->getDefaultWindowWidth() / 2 - 200, manager->getDefaultWindowHeight() / 2 - 85), 64, 64, Resources::MetroOff);
	buttons[2].second = PanelMap(manager, buttons[2].first.getPosition() - Vector2D(200, -20), Resources::CabezaVaporWave, 1, "D35P4C1T0", 1);
	buttons[0].second = PanelMap(manager, buttons[0].first.getPosition() - Vector2D(200, -20), Resources::CabezaPapito, 2, "Papito Daddy", 2);
	buttons[3].second = PanelMap(manager, buttons[3].first.getPosition() - Vector2D(-30, 110), Resources::EminemciaHead, 3, "Eminemcia", 3);
	buttons[4].second = PanelMap(manager, buttons[4].first.getPosition(), Resources::CabezaZombie, 4, "Corpselillo", 4);
	buttons[1].second = PanelMap(manager, buttons[1].first.getPosition(), Resources::AlienHead, 5, "Onilecram", 5);
